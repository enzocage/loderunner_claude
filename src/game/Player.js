...
export default class Player {
  ...
  _canMoveLeft(tileMap) {
    const nx = this.x - WALK_SPEED;
    const tx = Math.floor(nx / TILE_SIZE);
    const ty  = this.tileY;
    return tileMap.isPassable(tx, ty);
  }

  _canMoveRight(tileMap) {
    const nx = this.x + WALK_SPEED;
  ...
  update(input, tileMap) {
    ...
    // ── DIG ───────────────────────────────────────────────────────────────────
    if (input.digLeft && support === 'floor') {
      const dTx = this.tileX - 1;
      const dTy = this.tileY + 1;
      if (tileMap.get(dTx, dTy) === TILE.BRICK) {
    ...
    if (input.digRight && support === 'floor') {
      const dTx = this.tileX + 1;
      const dTy = this.tileY + 1;
      if (tileMap.get(dTx, dTy) === TILE.BRICK) {
        tileMap.startDig(dTx, dTy);
    ...
    if (input.up && onLadder && this._canMoveUp(tileMap)) {
      this.y -= CLIMB_SPEED;
    ...
    // ── WALK ──────────────────────────────────────────────────────────────────
    let moved = false;
    if (input.left && this._canMoveLeft(tileMap)) {
      this.x -= WALK_SPEED;
      this.facing = -1;
      this.state = S.WALK_LEFT;
      moved = true;
    } else if (input.right && this._canMoveRight(tileMap)) {
      this.x += WALK_SPEED;
      this.facing = 1;
